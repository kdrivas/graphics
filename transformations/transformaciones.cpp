...
GLfloat alfa=0.0, beta=0.0, theta=0.0, phi=0.0, gamma = 0.0, espiral= 0.0;
...
void animacion(){
	...
	alfa += (delta/25.0f) * glm::radians(1.0f);
	beta += (delta/25.0f) * glm::radians(1.0f);
	theta += (delta/25.0f) * glm::radians(1.0f);
	phi += (delta/25.0f) * glm::radians(1.0f);
	gamma += (delta/20.0f) * glm::radians(1.0f);
