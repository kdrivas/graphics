...
// Se creo una funcion de animacion la cual modifica la posicion de las luces
// basado en un angulo beta, el cual se va modificando
...
GLfloat t1 = 1.0f * glutGet(GLUT_ELAPSED_TIME);
	...
	uniform_mov = glGetUniformLocation(program, "mat_mov");
    if(uniform_mov == -1){
        std::cout << "No se puede asociar el atributo mov" << std::endl;
        return false;
    }
	
	uniform_mov1 = glGetUniformLocation(program, "mat_mov1");
    if(uniform_mov1 == -1){
        std::cout << "No se puede asociar el atributo mov" << std::endl;
        return false;
    }
...
void graficarObjeto(Mesh* mesh){
	...
	// las matrices de iluminacion se tranfieren al fragment shader a travez de las variables uniform creadas

	beta += (delta/5.0f) * glm::radians(1.0f);
	theta += (delta/10.0f) * glm::radians(1.0f);
	...
	glUniform4fv(uniform_mov,1,  glm::value_ptr(glm::vec4(0.0, 1.0, 2.0, 1.0) *glm::rotate(glm::mat4(1.0f), beta, glm::vec3(0.0f, 0.0f, 1.0f))));
	glUniform4fv(uniform_mov1,1,  glm::value_ptr(glm::vec4(2.0, 0.0, 0.0, 1.0) * glm::rotate(glm::mat4(1.0f), theta, glm::vec3(0.0f, 0.0f, 1.0f))));
